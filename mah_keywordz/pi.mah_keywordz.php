...
class Mah_keywordz
{
	...
	function Mah_keywordz($str = '')
	{
		...
		$return = array();
		$string = $sort = $option = $order = $limit = '';
		...
		$string = ($str == '') ? $this->EE->TMPL->fetch_param($prm_nms['string']) : $str;
		$string = ($string == '') ? $this->EE->TMPL->tagdata : $string;
		$sort = strtolower($this->EE->TMPL->fetch_param($prm_nms['sort']));
		$option = strtolower($this->EE->TMPL->fetch_param($prm_nms['option']));
		$order = strtolower($this->EE->TMPL->fetch_param($prm_nms['order']));
		$limit = intval($this->EE->TMPL->fetch_param($prm_nms['limit']));
		$default = $this->EE->TMPL->fetch_param($prm_nms['default']);
		
		// ----------------------------------
		// Return:
		// ----------------------------------
		
		$this->return_data = (!$string) ? $default : $this->make_keywordz($string, $sort, $option, $order, $limit);
	...
	function make_keywordz($string, $sort = 'false', $option = 'length', $order = 'descending', $limit = 0)
	{
		...
		$return = $this->remove_common_words($string);
		...
		if ($sort == 'true') {
			if ($option == 'length') { usort($return, array($this->class, "sort_by_key_length")); }
			if ($option == 'alphabetical') { usort($return, array($this->class, "sort_by_alphabet")); }
		}
		
		// ----------------------------------
		// Reverse the array and reset key indexes, or just reset key indexes:
		// ----------------------------------
		
		$return = ($order == 'ascending') ? array_reverse($return) : array_values($return);
	...
	function remove_common_words($string)
	{
	...
	function sort_by_key_length($a, $b)
	{
	...
	function usage()
	{
		...
		Please see forum thread for more information:
			...
			http://expressionengine.com/forums/viewthread/128406/
